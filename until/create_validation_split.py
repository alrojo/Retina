...
filenames = np.array(csv['image'])


# assert that no patient is in both train and validation subsets
patients_train = set([int(f.split("_")[0]) for f in filenames[idcs_train]])
patients_valid = set([int(f.split("_")[0]) for f in filenames[idcs_valid]])
