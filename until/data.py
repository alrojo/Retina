...
split = np.load("/home/ubuntu/jonathan_ebs/kaggle-eyes-master/data/split.pkl")

# bp()
csv = pd.read_csv("/home/ubuntu/jonathan_ebs/kaggle-eyes-master/data/trainLabels.csv")
filenames_train = np.array(csv['image'])
paths_train = np.array([os.path.join("/home/ubuntu/data_storage/data/flatten256", "%s.png" % filename) for filename in filenames_train])
#paths_train = np.array([os.path.join("/home/ubuntu/data_storage/data/cropped_256", "%s.png" % filename) for filename in filenames_train])
...
print "%s deleting paths to %d missing images%s" % ("\n"*3,sum,"\n"*3)



to_delete=[]
split_train = split['train'].tolist()
split_valid = split['valid'].tolist()
print "train and valid are:"
print len(split_train)
print len(split_valid)
print type(split_valid)
print "\n"*2
for i1, i2 in enumerate(paths_train): 
    if not os.path.exists(i2):
        print "deleting: %s" % i2
        to_delete.append(i1)
        if (i1 in split_train):
            split_train.remove(i1)
        if (i1 in split_valid):
            split_valid.remove(i1)
...
split['train'] = np.asarray(split_train)
split['valid'] = np.asarray(split_valid)
del split_train, split_valid

# filenames_train = np.delete(filenames_train,to_delete)
# paths_train = np.delete(filenames_train,to_delete)
# labels_train = np.delete(labels_train, to_delete)
