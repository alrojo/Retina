...
def build_model():
    ...
    l1a_c = Conv2DLayer(l_in, num_filters=6, filter_size=(3, 3), border_mode='same', W=nn.init.Orthogonal('relu'))
    l1b_c = Conv2DLayer(l1a_c, num_filters=12, filter_size=(3, 3), border_mode='same', W=nn.init.Orthogonal('relu'))
    l1_pool = MaxPool2DLayer(l1b_c, (2, 2), (2, 2))
    l2a_c = Conv2DLayer(l1_pool, num_filters=24, filter_size=(3, 3), border_mode='same', W=nn.init.Orthogonal('relu'))
    l2b_c = Conv2DLayer(l2a_c, num_filters=48, filter_size=(3, 3), border_mode='same', W=nn.init.Orthogonal('relu'))
    l2_pool = MaxPool2DLayer(l2b_c, (2, 2), (2, 2))

    l6 = nn.layers.DenseLayer(nn.layers.dropout(l2_pool, p=0.5), num_units=98, W=nn.init.Orthogonal('relu'))

    l_out = nn.layers.DenseLayer(nn.layers.dropout(l6, p=0.5), num_units=1, nonlinearity=None, W=nn.init.Orthogonal('relu'), b=nn.init.Constant(2))
